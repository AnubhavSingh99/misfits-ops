...
export declare const TIME_OF_DAY_OPTIONS: {
    value: TimeOfDay;
    label: string;
    time: string;
    icon: string;
...
export declare const CAPACITY_BUCKET_OPTIONS: CapacityBucket[];
...
export type ClubApplicationStatus = 'ACTIVE' | 'ABANDONED' | 'NOT_INTERESTED' | 'SUBMITTED' | 'UNDER_REVIEW' | 'ON_HOLD' | 'INTERVIEW_PENDING' | 'INTERVIEW_SCHEDULED' | 'INTERVIEW_DONE' | 'SELECTED' | 'CLUB_CREATED' | 'REJECTED';
export interface ScreeningRatings {
    [key: string]: number;
}
export type RejectionReason = 'insufficient_experience' | 'low_commitment' | 'unclear_motivation' | 'city_not_available' | 'incomplete_responses' | 'other';
