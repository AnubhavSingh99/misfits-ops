...
export const ACTIVITY_NAME_REVERSE: Record<string, string> = Object.fromEntries(
  Object.entries(ACTIVITY_NAME_MAP).map(([k, v]) => [v.toLowerCase(), k])
);

// All 159 rows from meetup master.csv
export const MEETUP_DEFAULTS: MeetupDefault[] = [
  // Gurgaon - GCR Extn.
 ...
 */
export function getMeetupDefaults(
  activity: string,
  city: string,
  area?: string
): MeetupDefaultsResponse {
  ...
  // 1. Try exact match (activity + city + area)
  if (normalizedArea) {
    const exactMatch = MEETUP_DEFAULTS.find(
      d => d.activity === normalizedActivity &&
  ...
  // 2. Fallback to city average (all areas in city for this activity)
  const cityMatches = MEETUP_DEFAULTS.filter(
    d => d.activity === normalizedActivity &&
         d.city.toLowerCase() === normalizedCity.toLowerCase()
  );
  ...
  // 3. Fallback to activity average (all cities for this activity)
  const activityMatches = MEETUP_DEFAULTS.filter(
    d => d.activity === normalizedActivity
  );
 ...
 */
export function calculateTargetRevenue(
  targetMeetups: number,
  meetupCost: number,
  meetupCapacity: number
): number {
