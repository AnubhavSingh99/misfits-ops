...
// Status configuration with Misfits-aligned colors
const STATUS_CONFIG = {
  not_picked: {
    label: 'Not Picked',
    shortLabel: 'NP',
...
interface HierarchyNode {
  ...
  requirements?: LeaderRequirement[];
}
...
export default function LeaderRequirementsDashboard({
  ...
  useEffect(() => {
    const fetchFilterOptions = async () => {
      try {
        const res = await fetch(`${API_BASE}/requirements/leaders/filter-options`);
  ...
  const fetchData = useCallback(async (showRefresh = false) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/requirements/leaders/hierarchy?${params}`);
      ...
      console.error('Failed to fetch leader requirements:', err);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
        ...
        >
          ...
          <td className="py-3 px-4 text-center">
            <button
              ...
              className="opacity-0 group-hover:opacity-100 p-1.5 rounded-md bg-indigo-50 text-indigo-600 hover:bg-indigo-100 transition-all"
            >
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-gray-400">
              ...
              <p className="text-lg font-medium">No leader requirements found</p>
...
interface SummaryTileProps {
  ...
  color: 'indigo' | 'slate' | 'blue' | 'emerald' | 'amber';
}
...
function SummaryTile({ label, count, icon: Icon, color }: SummaryTileProps) {
  ...
  return (
    <div className={`rounded-lg border border-gray-100 p-2 ${classes.bg} hover:shadow-md transition-shadow`}>
      <div className="flex items-center justify-between gap-1">
        <div>
          <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">{label}</p>
          <p className={`text-base font-bold ${classes.text}`}>{count}</p>
        </div>
        <div className={`p-1 rounded-md ${classes.icon}`}>
