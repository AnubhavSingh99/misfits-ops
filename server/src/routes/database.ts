...
/**
 * GET /api/database/revenue - Get current revenue data
 * Based on Club Health Report Generation Script calculations - EXACT METHODOLOGY
 */
router.get('/revenue', async (req, res) => {
  try {
    // Using exact logic from Club Health Report Generation Script
    const revenueQuery = `
      WITH week_boundaries AS (
        SELECT
          -- Current week boundaries in IST converted to UTC
          (DATE_TRUNC('week', (NOW() AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Kolkata') AT TIME ZONE 'Asia/Kolkata') AT TIME ZONE 'UTC' as current_week_start_utc,
          (DATE_TRUNC('week', (NOW() AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Kolkata' + INTERVAL '1 week') AT TIME ZONE 'Asia/Kolkata') AT TIME ZONE 'UTC' - INTERVAL '1 second' as current_week_end_utc,

          -- Last 3 months for revenue calculation
          DATE_TRUNC('month', CURRENT_DATE - INTERVAL '3 months') as three_months_ago
      ),
      monthly_revenue AS (
        SELECT
          ...
          SUM(CASE
            ...
            THEN b.amount::numeric / 100
            ELSE 0
          END) as total_revenue_rupees
        FROM event e
        ...
        WHERE e.state = 'CREATED'
          ...
          AND e.start_time >= wb.three_months_ago
    ...
    const result = await queryMisfits(revenueQuery);

    // Calculate current month revenue (latest entry)
    const currentMonthRevenue = result.rows[0]?.total_revenue_rupees || 0;

    // Calculate 3-month average for target calculation
    const threeMonthTotal = result.rows.reduce((sum: number, row: any) =>
      sum + (parseFloat(row.total_revenue_rupees) || 0), 0
    );
    const averageMonthly = threeMonthTotal / Math.max(result.rows.length, 1);

    // Target is 40% growth from current average
    const targetRevenue = averageMonthly * 1.4;
    ...
    logger.error('Revenue query error:', error);
    res.status(500).json({
      success: false,
